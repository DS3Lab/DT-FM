...
do
  ...
  then
    ...
    ssh -i ../YOUR_PEM_FILE.pem ubuntu@"$ip" "bash -s" < ./local_scripts/"${script}" "$master_ip" "$world_size" "$rank" 0 "$ga_step" "$num_layers" "$batch_size" "$log_mode" &
  elif [ $# -eq 6 ]
  then
    case=$6
    echo "Running in heterogeneous network: Case-$case"
    ssh -i ../YOUR_PEM_FILE.pem ubuntu@"$ip" "bash -s" < ./local_scripts/"${script}" "$master_ip" "$world_size" "$rank" 0 "$ga_step" "$num_layers" "$batch_size" "$log_mode" "$case" &
  elif [ $# -eq 7 ]
  then
    delay_ms=$6
    rate_gbit=$7
    ssh -i ../YOUR_PEM_FILE.pem ubuntu@"$ip" "bash -s" < ./local_scripts/"${script}" "$master_ip" "$world_size" "$rank" 0 "$ga_step" "$num_layers" "$batch_size" "$log_mode" "$delay_ms" "$rate_gbit" &
  else
